...
class ToolsControllerTest extends \Zend\Test\PHPUnit\Controller\AbstractConsoleControllerTestCase
{
    ...
    'no_controller-no_action' => $oAssetsBundleService->getCacheFileName(\AssetsBundle\Service\ServiceOptions::NO_MODULE, \AssetsBundle\Service\ServiceOptions::NO_CONTROLLER, \AssetsBundle\Service\ServiceOptions::NO_ACTION),
            ...
            $this->assertStringEqualsFile(
                    $sCacheExpectedPath . DIRECTORY_SEPARATOR . $sCacheFile . '.css', preg_replace('/cache\/([0-9a-f]{32})\//', 'cache/encrypted-file-tree/', file_get_contents($oAssetsBundleService->getOptions()->getCachePath() . $sCacheFile . '.css')), $sCachePart . ' - ' . $sCacheFile . '.css'
            );

            //Less cache files
            $this->assertStringEqualsFile(
                    $sCacheExpectedPath . DIRECTORY_SEPARATOR . $sCacheFile . '.less', preg_replace('/cache\/([0-9a-f]{32})\//', 'cache/encrypted-file-tree/', file_get_contents($oAssetsBundleService->getOptions()->getCachePath() . $sCacheFile . '.less')), $sCachePart . ' - ' . $sCacheFile . '.less'
            );

            //Js cache files
            $this->assertFileEquals(
                    $sCacheExpectedPath . DIRECTORY_SEPARATOR . $sCacheFile . '.js', $oAssetsBundleService->getOptions()->getCachePath() . $sCacheFile . '.js', $sCachePart . ' - ' . $sCacheFile . '.js'
            );
        }
    }
