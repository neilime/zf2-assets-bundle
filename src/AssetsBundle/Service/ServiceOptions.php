...
class ServiceOptions extends \Zend\Stdlib\AbstractOptions
{
    ...
    public function setCachePath($sCachePath)
    {
        if (is_string($sCachePath)) {
            if (!is_dir($sRealCachePath = $this->getRealPath($sCachePath))) {
                ...
                $this->cachePath = $sRealCachePath . DIRECTORY_SEPARATOR;
            }
            return $this;
        }
    ...
    public function getRealPath($sPath)
    {
        if (empty($sPath) || !is_string($sPath)) {
            throw new \InvalidArgumentException('Path is not valid : ' . gettype($sPath));
        }
        if (isset($this->resolvedPaths[$sPath]) && file_exists($this->resolvedPaths[$sPath])) {
            return $this->resolvedPaths[$sPath];
        }

        //If path is "/", assets path is prefered
        if ($sPath === '/' && $this->hasAssetsPath()) {
            return $this->resolvedPaths[$sPath] = $this->getAssetsPath();
        }
        ...
        if (file_exists($sPath)) {
            if (strpos($sPath, 'phar://') !== 0) {
                return $this->resolvedPaths[$sPath] = realpath($sPath);
            }
            $sPath = substr($sPath, 7);

            // whether $sPath is unix or not
            $unipath = strlen($sPath) == 0 || $sPath{0} != '/';
            // attempts to detect if path is relative in which case, add cwd
            if (strpos($sPath, ':') === false && $unipath) {
                $sPath = getcwd() . DIRECTORY_SEPARATOR . $sPath;
            }
            // resolve path parts (single dot, double dot and double delimiters)
            $sPath = str_replace(array('/', '\\'), DIRECTORY_SEPARATOR, $sPath);
            $aAbsolutes = array();
            foreach (array_filter(explode(DIRECTORY_SEPARATOR, $sPath), 'strlen') as $sPart) {
                if ('.' == $sPart) {
                    continue;
                }
                if ('..' == $sPart) {
            ...
            $sPath = implode(DIRECTORY_SEPARATOR, $aAbsolutes);
            // resolve any symlinks
            if (file_exists($sPath) && linkinfo($sPath) > 0) {
                $sPath = readlink($sPath);
            }
            // put initial separator that could have been lost
            $sPath = !$unipath ? '/' . $sPath : $sPath;
            return $this->resolvedPaths[$sPath] = 'phar://' . $sPath;
        }

        if (strpos($sPath, '@zfRootPath') !== false) {
            $sPath = str_ireplace('@zfRootPath', getcwd(), $sPath);
        }
        if (strpos($sPath, '@zfAssetsPath') !== false) {
            $sPath = str_ireplace('@zfAssetsPath', $this->getAssetsPath(), $sPath);
        }

        if (($sRealPath = realpath($sPath)) !== false) {
            return $this->resolvedPaths[$sOriginalPath] = $sRealPath;
        }

        //Try to guess real path with root path or asset path (if defined)
        if (file_exists($sRealPath = getcwd() . DIRECTORY_SEPARATOR . $sPath)) {
            return $this->resolvedPaths[$sOriginalPath] = realpath($sRealPath);
        }
        if ($this->hasAssetsPath() && file_exists($sRealPath = $this->getAssetsPath() . $sPath)) {
            return $this->resolvedPaths[$sOriginalPath] = realpath($sRealPath);
        }
        return false;
    }
     ...
     * @throws \InvalidArgumentException
     */
    public function sanitizeFileName($sFilename)
    {
        if (!is_string($sFilename)) {
            throw new \InvalidArgumentException('Filename expects string, "' . gettype($sFilename) . '" given');
        }
        $sFilename = str_replace(array("?", "[", "]", "/", "\\", "=", "<", ">", ":", ";", ",", "'", "\"", "&", "$", "#", "*", "(", ")", "|", "~", "`", "!", "{", "}"), '', $sFilename);
        $sFilename = preg_replace('/[\s-]+/', '-', $sFilename);
        return trim($sFilename, '.-_');
    }
